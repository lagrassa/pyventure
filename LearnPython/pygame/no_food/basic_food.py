...
class Food(object):
	...
	def update(self,dt,screen,snake):
		...
		for f_piece in self.food :
			if f_piece.middle_x == snake.x and f_piece.middle_y == snake.y : #if the food and snake are in the same place
                                #TODO LENGTH increase the length of the snake and give it some number of points
                                # call the method on the snake object
                                # use whatever parameters you programmed into basic_player.py
                                # call remove(food piece) on the food object so the food disappears. 
                                # you want to move the piece of food
                                print "Haha! f_piece is still there!"
			f_piece.blit(screen)
